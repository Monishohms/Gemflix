...
function ModalContent({
  ...
  // const closeModel = onClose;
  const dispatch = useDispatch();
  //const status = useSelector((store) => store.modal.showModal);
  ...
  return (
    <div className="flex justify-center items-center absolute">
      <div
        className={
          " bg-black drop-shadow-sm  mx-auto h-[25%] w-[50%] m-1 p-1  rounded-lg aspect-video  border border-gray-400 overflow-y-scroll overflow-x-hidden no-scrollbar-custom"
        }
      >
        {play === true ? (
        ...
        <div>
          <i className="fixed top-10 right-1 sm:right-5 md:right-10 opacity-90 text-3xl sm:text-4xl fa-solid fa-circle-xmark fa-shake cursor-pointer">
            <IoCloseCircleOutline className="cursor-pointer  text-4xl text-white" />
          </i>
        </div>
      </div>
